...
export default function EventManage(props: any) {
  ...
  // -----------------Start: useState-----------------
  var [eventDetailForUpdate, setEventDetailForUpdate] = useState<any>(null);
  var [eventDetailForRender, setEventDetailForRender] = useState<any>(null);
      ...
      if (qrcodeList.includes(dataFromChild)) {
        ...
        try {
          fetchWrapper.post("/api/UserTickets/Check-in", {
            id: dataFromChild,
            eventId: eventDetailForRender?.id
  ...
  useEffect(() => {
    ...
    fetchWrapper.get(`api/Event/detail-by-id?id=${id}`)
      .then(data => {
        setEventDetailForUpdate(data)
        setEventDetailForRender(data);
        let startDateTemp = new Date(data.startDate);
        let endDateTemp = new Date(data.endDate);
        console.log(endDateTemp.toTimeString().split('T')[0].split(' ')[0])
        setStartDate({
          date: startDateTemp.toISOString().split('T')[0],
          time: startDateTemp.toTimeString().split('T')[0].split(' ')[0]
        })
        setEndDate({
          date: endDateTemp.toISOString().split('T')[0],
          time: endDateTemp.toTimeString().split('T')[0].split(' ')[0]
        })
        ...
        {eventDetailForRender ? (
          <div className="spread gap-2 mb-2 flex justify-between items-center">
            <h1 className="tab-title text-4xl font-semibold  dark:text-[#fff] mb-0">
              <div>
                <div className="inline">{eventDetailForRender.name}</div>
              </div>
            </h1>
            <Link
                ...
                <div className="relative">
                  ...
                  <div className="url-wrapper absolute p-2 rounded-lg bg-[rgba(19,21,23,0.32)] z-50 bottom-2 left-2 right-2 backdrop-blur-lg shadow-md text-sm">
                    <div className="url amimated transition-all duration-300 ease-in-out gap-2 flex justify-between items-center w-full">
                      <Link
                        href={`/ user / join - event / ${eventDetailForRender?.id}`}
                        target="_blank"
                      ...
                      >
                        <div className="text-sm w-full truncate" id="myClipboard">
                          {`user/join-event/${eventDetailForRender?.id}`}
                        </div>
                      ...
                      >
                        ...
                        <div className="label" onClick={(e) => navigator.clipboard.writeText(`${currentURL}/user/join-event/${eventDetailForRender?.id}`)}>SAO CHÉP</div>
                      </button>
                    </div>
                  </div>
                </div>
                      ...
                      <div className="gap-4 flex items-start w-full">
                        <div className="calendar-card-wrapper flex">
                          <div className="calendar-card border border-solid border-[rgba(19,21,23,0.08)] dark:border-[rgba(255,255,255,0.08)] rounded-lg overflow-hidden bg-[#fff] dark:bg-[#212325] w-11 h-11">
                            <div className="month bg-[rgba(19,21,23,0.08)] dark:bg-[rgba(255,255,255,0.08)] text-[0.625rem] font-semibold uppercase text-center text-[#b3b5b7] dark:text-[#939597]">{ConvertDateTime(eventDetailForRender.startDate).month}</div>
                            <div className="day text-lg text-center">{ConvertDateTime(eventDetailForRender.startDate).day}</div>
                          </div>
                        </div>
                        <div>
                          <div className="font-medium">{DayOfWeek(CovertDate(eventDetailForRender.startDate)[0])}</div>
                          <div className="text-sm text-[#737577] dark:text-[#d2d4d7] text-ellipsis truncate">{ConvertDateTime(eventDetailForRender.startDate).day} tháng {ConvertDateTime(eventDetailForRender.startDate).month}, {ConvertDateTime(eventDetailForRender.startDate).hour}:{ConvertDateTime(eventDetailForRender.startDate).minute} - {ConvertDateTime(eventDetailForRender.endDate).day} tháng {ConvertDateTime(eventDetailForRender.endDate).month}, {ConvertDateTime(eventDetailForRender.endDate).hour}:{ConvertDateTime(eventDetailForRender.endDate).minute} GMT+7</div>
                        </div>
                      </div>
                      <div>
                        <div className="gap-4 flex items-center">
                          ...
                          <div>
                            {
                              eventDetailForRender.isOnline ? (
                                <><Link href={eventDetailForRender.linkMeet} className="">{truncatedLinkMeet}</Link></>

                              ) :
                                (
                                  eventDetailForRender.addressName ? (
                                    <>  <div className="font-medium">{eventDetailForRender.addressName}</div></>
                                  ) : (
                ...
                <ModalBody>
                  ...
                  <div className="event-name w-full pb-2 mt-6">
                    <Input
                      ...
                      label="Tên sự kiện"
                      value={eventDetailForUpdate.name}
                      onChange={(e) =>
                        setEventDetailForUpdate({
                          ...eventDetailForUpdate,
                          name: e.target.value,
                  ...
                  <div className="desc w-full pb-2">
                    <Textarea
                      value={eventDetailForUpdate.description}
                      onChange={(e) =>
                        setEventDetailForUpdate({
                          ...eventDetailForUpdate,
